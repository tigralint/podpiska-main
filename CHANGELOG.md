...
- **📡 Полноценный Народный радар**: Лента алертов переведена на реальную БД (Upstash Redis). Теперь любые жалобы защищены Turnstile-капчей и в ту же секунду дублируются в Telegram. Добавлена живая фильтрация.
- **📚 Расширенная База Знаний**: Внедрен умный поиск для быстрого нахождения сервисов (с поддержкой алиасов). Каталог пополнен новыми инструкциями для Apple Music, Spotify и популярных EdTech школ.
- **🛡️ Защита от накруток**: Новый эндпоинт `reportPattern` с Rate Limiting и валидацией через Zod.
